/*******************************************************************************
 * ===========================================================================
 * Looper.js | a jQuery plugin - v1.1.1 Copyright 2013 Ry Racherbaumer
 * http://rygine.com/projects/looper.js Licensed under the Apache License,
...
(function($, window, document, undefined) {
  ...
  var Looper = function(element, options) {
    ...
    this.options.pause === 'hover'
            && this.$element.on('mouseenter', $.proxy(this.pause, this)).on(
                    'mouseleave', $.proxy(this.loop, this));
    ...
    go: function(to) {
      ...
      if (this.$element.hasClass('slide') || this.$element.hasClass('xfade')) {
        ...
        if (cssTransitionSupport) {
          ...
          // setup animations
          var active = {}, activeStyle, next = {}, nextStyle;
  ...
  $.fn.looper = function(option) {
    ...
    return this
            .each(function() {
              var $this = $(this), looper = $this.data('looperjs'), options = $
                      .extend({}, $.fn.looper.defaults,
                              typeof option == 'object' && option), action = typeof option == 'string'
                      ? option : option.looper, args = option.args
                      || (looperArgs.length > 1 && Array.prototype.slice.call(
                              looperArgs, 1));

              // ensure looper plugin
              if (!looper)
                $this.data('looperjs', (looper = new Looper(this, options)));
              ...
              // execute method if string
                ...
                // with arguments
                if (args)
                  looper[action].apply(looper, args.length ? args : ('' + args)
                          .split(','));
